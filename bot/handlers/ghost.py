...
async def _send_picker(message: Message, storage: Storage, owner_id: int, connection_id: str) -> None:
    ...
    if not enriched:
        await message.answer(
            "Пока нет ни одного чата с сохранённой историей. Включите «сохранять все сообщения» "
            "в /admin, либо просто дождитесь новых сообщений — режим призрака сам начнёт копить историю."
        )
        return
